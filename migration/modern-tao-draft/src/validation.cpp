...
{
    return tao::GetBlockSubsidy(nHeight, consensusParams);
}
...
{
    // TAO Revival mainnet genesis (2017) is later than the historical BIP16
    // switch time (2012), so P2SH applies to every valid TAO mainnet block.
    // Revival did not activate DERSIG/BIP66, CLTV/BIP65, CSV, SegWit or Taproot.
    (void)block_index;
    (void)chainman;
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, BlockManager& blockman, const ChainstateManager& chainman, const CBlockIndex* pindexPrev) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    // TAO Revival: reject block version 1 from mainnet height 710000.
    if (nHeight >= 710000 && block.nVersion < 2) {
        return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER,
                             "bad-version",
                             "TAO Revival requires block version >= 2 from height 710000");
    }
    ...
    // TAO Revival default checkpoint at height 1.
    // The legacy client allowed -checkpoints=0; configurability can be restored
    // during integration, but the default historical chain is preserved here.
    if (nHeight == 1 &&
        block.GetHash() != uint256{"717fc5ef97ded9f7d514fcb1ed558bc76596ce85f09890efa6e668d228c4a57f"}) {
        return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER,
                             "checkpoint-mismatch",
                             "TAO Revival height-1 checkpoint mismatch");
    }
