...
#if defined(NDEBUG)
# error "TaoCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x41de2a4b7018115eae4a3e7e82189b551aaf871af66929fa28a49ee852abb67e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // TaoCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "TaoCoin Signed Message:\n";
...
{
    // TaoCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // TaoCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // TaoCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1513853104, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Betoota Advocate 21Dec2017 Bloke That Got Caught Up Talking Bitcoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04defb2ebff8d065d863eeb265f549bf0d89a6f21475a117b85380802bf0480bd6ab7685e84aadfa3316b803ec74d251a175b37713232e5ec48ed7b0b486d3e3bc") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1513853104;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1349876;
        ...
        assert(block.hashMerkleRoot == uint256("0xcb8d58accf6e38658164002f3258e6f9123206997d234537aa725898c1fb1c62"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TaoCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TaoCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TaoCoinMiner(CWallet *pwallet)
{
    printf("TaoCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("taocoin-miner");
    ...
    try { loop {
        ...
        printf("Running TaoCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TaoCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TaoCoinMiner, pwallet));
}
