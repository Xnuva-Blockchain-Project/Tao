...
public:
    ...
    const BIGNUM* cget() const { return pbn; }
    ...
    CBigNum& SetCompact(unsigned int nCompact)
    {
        ...
        {
            nWord >>= 8*(3-nSize);
            BN_set_word(pbn, nWord);
        }
        else
        {
            BN_set_word(pbn, nWord);
            BN_lshift(pbn, pbn, 8*(nSize-3));
        }
        BN_set_negative(pbn, fNegative);
        return *this;
    }

    unsigned int GetCompact() const
    {
        unsigned int nSize = BN_num_bytes(pbn);
        unsigned int nCompact = 0;
        if (nSize <= 3)
            nCompact = BN_get_word(pbn) << 8*(3-nSize);
        else
        {
            CBigNum bn;
            BN_rshift(bn.get(), pbn, 8*(nSize-3));
        ...
        nCompact |= nSize << 24;
        nCompact |= (BN_is_negative(pbn) ? 0x00800000 : 0);
        return nCompact;
    }
    ...
    std::string ToString(int nBase=10) const
    {
        ...
        CBigNum dv;
        CBigNum rem;
        if (BN_cmp(bn.cget(), bn0.cget()) == 0)
            return "0";
        while (BN_cmp(bn.cget(), bn0.cget()) > 0)
        {
    ...
    CBigNum& operator--()
    {
        // prefix operator
        CBigNum r;
        if (!BN_sub(r.get(), pbn, BN_value_one()))
